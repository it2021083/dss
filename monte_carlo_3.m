...
% Plotting the PRR as a function of perturbation strength on a logarithmic scale
figure;
hold on;
colors = {'r', 'g', 'b', 'k'};  % Different colors for each alternative
markers = {'o', 's', 'd', '^'}; % Different markers for each alternative
for i = 1:alternatives
    % Use semilogy to plot the y-axis (PRR) on a logarithmic scale
    semilogy(s_values, PRR(:, i), '-o', 'Color', colors{i}, 'Marker', markers{i}, 'LineWidth', 1.5, ...
         'MarkerSize', 6, 'DisplayName', sprintf('Alternative %d', i));
end
xlabel('Perturbation Strength (s)');
ylabel('Probability of Rank Reversal (PRR)');
title('Sensitivity Analysis using Monte Carlo Simulation (Logarithmic Scale)');
legend('Location', 'Best');
xlim([min(s_values)-0.1, max(s_values)+0.1]);  % Adjust x-axis limits slightly
ylim([1e-4, 1]);  % Set y-axis limits to cover a wide range (adjust as needed)
grid on;
hold off;
